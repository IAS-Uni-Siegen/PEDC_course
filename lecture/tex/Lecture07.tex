\section{Passive components in power electronics- Magnetics}
\title[Passive components- Magnetics]{Passive components- Magnetics}  
...
\begin{frame}
    ...
    \begin{columns}
        \begin{column}{0.58\textwidth}
            As in the case of the single winding filter inductor, the size of the minor B-H loop is proportional to the total current ripple. Small ripple implies small core loss, as well as small proximity loss. An air gap is employed, and the maximum flux density is typically limited by saturation.
        \end{column}
...
\begin{frame}
    ...
    \begin{columns}
        \begin{column}{0.5\textwidth}
            \begin{figure}
                \centering
                \includegraphics[width=0.85\linewidth]{fig/lec07/EE_core.png}
                \includegraphics[width=0.85\linewidth]{fig/lec07/ETD_core.png}
        ...
        \begin{column}{0.5\textwidth}
            \begin{figure}
                \includegraphics[width=0.85\linewidth]{fig/lec07/PQ_core.png}
                \includegraphics[width=0.85\linewidth]{fig/lec07/RM_core.png}
...
\begin{frame}
    ...
    \begin{tikzpicture}[
        ...
        \node[block, minimum width=3.0cm] (total) at (0,0) 
        {Total magnetic\\component loss};

        \node[block] (core) at (-4.2,-1.5) {Core loss};
        \node[block] (copper) at (0,-1.5) {Copper loss};
        \node[block] (parasitic) at (4.2,-1.5) {Parasitic-related\\loss};

        \node[block] (hyst) at (-5.5,-3.0) {Hysteresis\\loss};
        \node[block] (eddy) at (-2.9,-3.0) {Core eddy\\current loss};

        \node[block] (dc) at (-1.3,-3.0) {DC winding\\loss};
        \node[block] (ac) at (1.3,-3.0) {AC winding\\loss};

        \node[block] (skin) at (0.0,-4.5) {Skin effect};
        \node[block] (prox) at (2.6,-4.5) {Proximity effect};

        \draw[arrow] (total) -- (core);
        \draw[arrow] (total) -- (copper);
        \draw[arrow] (total) -- (parasitic);

        \draw[arrow] (core) -- (hyst);
        \draw[arrow] (core) -- (eddy);

        \draw[arrow] (copper) -- (dc);
        \draw[arrow] (copper) -- (ac);
...
\begin{frame}
    \frametitle{Fringing-Field Loss in Gapped Inductors}
            \begin{figure}
                \centering
                \includegraphics[width=0.35\linewidth]{fig/lec07/fringing_gap_cases.png}
